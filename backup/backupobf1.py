...
def mode3(file):
    # Code supplémentaire à ajouter avant l'obfuscation
    additional_code = """
# Fonction de génération de clé aléatoire
def generate_key(length):
    return ''.join(random.choice(string.ascii_letters + string.digits) for _ in range(length))

# Génération de la clé pour le déchiffrement
key = generate_key(16)

# Mode Debug
DEBUG_MODE = False

# Fonction pour exécuter le code en mode Debug
def debug():
    if DEBUG_MODE:
        print("Debug mode activated!")
        # Ajoutez ici du code pour afficher des informations supplémentaires pendant le débogage
    else:
        return

# Exécution du mode Debug
debug()
"""

    # Ajout du code supplémentaire au début du fichier
    x = open(file).read()
    x = additional_code + x
    ...
    m = compile(x, '', 'exec')
    k = marshal.dumps(m)
    l = open('secure-' + file, 'w')
    l.write('import marshal\n')
    l.write('exec(marshal.loads(' + repr(k) + "))")
    l.close()

    print('File encrypted successfully as \'secure-{}\''.format(file))
...
+-------------------------------------------------------------------+
 """ + clear)
    ...
    if number == 1:
        ...
        file = input("Enter the name of the file to be secured: ")
        mode3(file)
